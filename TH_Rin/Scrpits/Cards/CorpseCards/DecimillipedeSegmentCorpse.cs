...
public class DecimillipedeSegmentCorpse : CorpseCardModel
{
	...
	public DecimillipedeSegmentCorpse() : base(-1, CardType.Status, CardRarity.Event, TargetType.None)
	{
		RefreshCorpseVars();
	}
	protected override void RefreshCorpseVars()
	{
		DynamicVars["Power"].BaseValue = 66m * GetMutilplier();
     ...
     public override async Task TriggerWhenTurnEnd(PlayerChoiceContext choiceContext, CombatSide side)
        {
			...
			await CreatureCmd.Heal(Owner.Creature,(Owner.Creature.MaxHp-Owner.Creature.CurrentHp)* this.DynamicVars["Power"].IntValue/100);
		}
