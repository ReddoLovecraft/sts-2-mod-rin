...
public class ChomperCorpse : CorpseCardModel
{
	...
	public ChomperCorpse() : base(-1, CardType.Status, CardRarity.Event, TargetType.None)
	{
		RefreshCorpseVars();
	}
	...
	public override async Task TriggerWhenTurnStart(PlayerChoiceContext choiceContext, Player player)
    {
          if (player != base.Owner)
			...
			foreach(Creature mos in Owner.Creature.CombatState.HittableEnemies)
			{
				if(mos.IsAlive)
				{
					await PowerCmd.Apply<PiercingWailPower>(mos, this.DynamicVars.Cards.IntValue,Owner.Creature, this);
				}
			}
    }
