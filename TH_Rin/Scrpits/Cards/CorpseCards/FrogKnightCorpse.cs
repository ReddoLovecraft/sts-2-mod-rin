...
public class FrogKnightCorpse : CorpseCardModel
{
	...
	public FrogKnightCorpse() : base(-1, CardType.Status, CardRarity.Event, TargetType.None)
	{
		RefreshCorpseVars();
	}
	protected override void RefreshCorpseVars()
	{
		DynamicVars["Power"].BaseValue = 19m * GetMutilplier();
		DynamicVars.Cards.BaseValue = RotCount;
	}	
	 public override async Task TriggerWhenCombatStart()
        {
          await PowerCmd.Apply<PlatingPower>(Owner.Creature, this.DynamicVars["Power"].IntValue,Owner.Creature,this);
        }
