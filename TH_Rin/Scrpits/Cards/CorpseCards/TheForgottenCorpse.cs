...
public class TheForgottenCorpse : CorpseCardModel
{
	...
	public TheForgottenCorpse() : base(-1, CardType.Status, CardRarity.Event, TargetType.None)
	{
		RefreshCorpseVars();
	}
	 	...
	 	public override async Task TriggerWhenTurnStart(PlayerChoiceContext choiceContext, Player player)
    {
		...
		foreach(Creature mos in Owner.Creature.CombatState.HittableEnemies)
		{
			if(mos.IsAlive)
			{
				await PowerCmd.Apply<StrengthPower>(mos,-DynamicVars.Cards.BaseValue,Owner.Creature,this);
				await PowerCmd.Apply<StrengthPower>(Owner.Creature,DynamicVars.Cards.BaseValue,Owner.Creature,this);
			}
		}
	}
