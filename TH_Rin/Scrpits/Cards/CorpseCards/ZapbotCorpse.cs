...
public class ZapbotCorpse : CorpseCardModel
{
	...
	public ZapbotCorpse() : base(-1, CardType.Status, CardRarity.Event, TargetType.None)
	{
		RefreshCorpseVars();
	}
	...
	public override async Task TriggerWhenTurnEnd(PlayerChoiceContext choiceContext, CombatSide side)
        {
			...
			foreach(Creature creature in base.Owner.Creature.CombatState.HittableEnemies.ToList())
			  if(creature.IsAlive)
			  await CreatureCmd.Damage(choiceContext,creature, DynamicVars.Cards.IntValue,ValueProp.Unpowered,null,null);
		}
