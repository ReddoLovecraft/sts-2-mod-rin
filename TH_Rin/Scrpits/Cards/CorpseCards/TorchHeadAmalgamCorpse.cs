...
public class TorchHeadAmalgamCorpse : CorpseCardModel
{
	...
	public TorchHeadAmalgamCorpse() : base(-1, CardType.Status, CardRarity.Event, TargetType.None)
	{
		RefreshCorpseVars();
	}
	...
	public override async Task TriggerWhenTurnStart(PlayerChoiceContext choiceContext, Player player)
    {
		...
		if (creature != null)
		{
			...
			{
				await CreatureCmd.Damage(choiceContext,creature, DynamicVars["HpLoss"].BaseValue,ValueProp.Unpowered,null,null);
				if(!creature.IsAlive)
				{
                  break;
				}
