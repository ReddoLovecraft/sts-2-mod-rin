...
public class MyteCorpse : CorpseCardModel
{
	...
	public MyteCorpse() : base(-1, CardType.Status, CardRarity.Event, TargetType.None)
	{
		RefreshCorpseVars();
	}
	protected override void RefreshCorpseVars()
	{
	 ...
	 public override async Task TriggerWhenTurnStart(PlayerChoiceContext choiceContext, Player player)
	 {
		...
		if (creature != null)
		{
			NGoopyImpactVfx nGoopyImpactVfx = NGoopyImpactVfx.Create(creature);
			if (nGoopyImpactVfx != null)
			{
				NCombatRoom.Instance?.CombatVfxContainer.AddChildSafely(nGoopyImpactVfx);
			}
			await PowerCmd.Apply<PoisonPower>(creature, DynamicVars["Power"].BaseValue,Owner.Creature,this);
		}
		Creature creature2 = base.Owner.RunState.Rng.CombatTargets.NextItem(base.Owner.Creature.CombatState.HittableEnemies);
		if (creature2 != null)
		{
			NGoopyImpactVfx nGoopyImpactVfx = NGoopyImpactVfx.Create(creature2);
			if (nGoopyImpactVfx != null)
			{
				NCombatRoom.Instance?.CombatVfxContainer.AddChildSafely(nGoopyImpactVfx);
			}
			await PowerCmd.Apply<PoisonPower>(creature2, DynamicVars["Power"].BaseValue,Owner.Creature,this);
		}
	 }
